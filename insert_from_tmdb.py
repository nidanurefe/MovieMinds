...
def get_movies(page=1)->dict:
    url:str = f"{BASE_URL}/discover/movie"
    headers:list = {
...
def get_movie_cast(movie_id)->list:
    url:str= f"{BASE_URL}/movie/{movie_id}/credits"
    headers = {
        "Authorization": f"Bearer {API_KEY}",
        "accept": "application/json"
    }
...
def get_genres()->list:
    url:str = f"{BASE_URL}/genre/movie/list"
    headers:list = {
        "Authorization": f"Bearer {API_KEY}",
        "accept": "application/json"
    }
...
def get_movie_details(movie_id)->dict:
    url:str = f"{BASE_URL}/movie/{movie_id}"
    headers:list = {
        "Authorization": f"Bearer {API_KEY}",
        "accept": "application/json"
    }
...
def insert_movie(connection, movie) -> None:
    with connection.cursor() as cursor:  
        genre_id:int = movie['genre_ids'][0] if movie['genre_ids'] else None
        movie_details:dict = get_movie_details(movie['id'])
        if movie_details:
        ...
        poster_path:str = IMAGE_BASE_URL + movie.get('poster_path', '')

        check_sql = "SELECT COUNT(*) AS count FROM movies WHERE tmdb_id = %s"
        cursor.execute(check_sql, (movie['id'],))
        result:dict = cursor.fetchone()

        count:int = result['count'] if isinstance(result, dict) else result[0]
...
def insert_actor(connection, actor_id, actor_name, img_url=None) -> None:
    img_url:str= IMAGE_BASE_URL + img_url if img_url else None
...
def link_movie_producers(connection, movie_id)->None:
    movie_details:dict = get_movie_details(movie_id)
    if not movie_details:
        return
    producers:dict = movie_details.get('production_companies', [])
    for producer in producers:
        producer_id:int = producer['id']
        producer_name:int = producer['name']
        insert_producer(connection, producer_id, producer_name)
...
def main():
    connection:pymysql.Connection = get_db_connection(app)
    insert_all_genres(connection)
    movies_data:dict = get_movies(page=1)
    ...
    if movies_data:
        for movie in movies_data['results']:
            ...
            for movie in movies_data['results']:
                ...
                cast:dict = get_movie_cast(movie['id'])
                for actor in cast:
                    actor_id:int = actor.get('id')
                    actor_name:int = actor.get('name')
                    img_url:str = actor.get('profile_path', None)
